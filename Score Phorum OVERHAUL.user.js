...
function rebuildPosts() {
    if (debug)
        console.log("Start tvorby struktury");
    var posts = loadPosts();
    ...
    for (i = 0; i < posts.length; i++) {
...

        {
            ...
            if ((shouldBeHidden(post, 'user') && blockedUserLevel() === 1) || (shouldBeHidden(post, 'reaction') && blockedReactionLevel() === 1)) {
            ...
            if (pictures.length > 0)
            {
                for (p = 0; p < pictures.length; p++)
                {
                    ...
                    pictures[p].className += " picture ";
                    if (localStorage.getItem("nsfw") === "Y") {
                        pictures[p].style.width = "30px";
                    }
                }
            }
        }
        ...
        if ((shouldBeHidden(post, 'user') && blockedUserLevel() === 2) || (shouldBeHidden(post, 'reaction') && blockedReactionLevel() === 2)) {
            hideMore.push(postList);
            if ((shouldBeHidden(posts[i + 1], 'user') && blockedUserLevel() === 2) || (shouldBeHidden(posts[i + 1], 'reaction') && blockedReactionLevel() === 2)) {
                //přidá se i další
            } else {
                appendHidden(hideMore, containerDiv);
                hideMore = [];
            }
        } else {
            if ((shouldBeHidden(post, 'user') && blockedUserLevel() === 3) || (shouldBeHidden(post, 'reaction') && blockedReactionLevel() === 3)) {
                postList.className += " total_hide";
                var style = "height: 0;overflow: hidden;display: block;min-height: 0;margin: 0px;";
                if (post.root && (posts[i - 1] === undefined || (posts[i - 1] && posts[i - 1].root))) {
                    style += "border-bottom: 3px solid rgb(200, 196, 160);margin-bottom: 5px;margin-left:45px";
                }
                postList.setAttribute("style", style);
            }
    ...
    if (debug)
        console.log("Struktura hotova");
}

function blockedUserLevel() {
    return parseInt(window.localStorage.getItem("blocked_user_level"));
}

function blockedReactionLevel() {
    return parseInt(window.localStorage.getItem("blocked_reaction_level"));
}

function appendHidden(hideMore, containerDiv) {

    var div = document.createElement("div");
    var show = document.createElement("div");
    var hide = document.createElement("div");
    show.className = "show info_line";
    var style = "text-align: center;cursor:pointer;height: 7px;line-height: 7px;font-size: 8px;margin-bottom: 5px;background-color: rgba(200, 196, 160, 0.3);-webkit-box-shadow:0 0 3px rgba(51, 51, 51, 0.31);box-shadow: 0 0 3px rgba(51, 51, 51, 0.31);";
    if (hideMore[0].className.indexOf("reply") === -1) {
        style += "margin-left:0px;";
    }
    show.setAttribute("style", style);
    show.setAttribute("onClick", "this.nextSibling.style.height = 'auto';this.style.display = 'none';return false;");
    show.innerHTML = hideMore.length;
    hide.setAttribute("style", "opacity: 0.5;height: 0;overflow: hidden;");
    for (var i = 0; i < hideMore.length; i++) {
        hide.appendChild(hideMore[i]);
    }
...
function shouldBeHidden(post, who) {
    ...
    if (!blockedUsers.every(isOk) || !spammers.every(isOk) || detectSpam(post)) {
        return true;
    }
    return false;
}
function loadRooms() {
    if (debug)
        console.log("Načtení všech místností");
...
{
    if (debug)
        console.log("Načtení oblíbených místností");
...
function settings() {
    ...
    var blockedUsersLevel, blockedUsersLevelHeader;
    {
        blockedUsersLevelHeader = document.createElement("h2");
        blockedUsersLevelHeader.setAttribute("class", "submenu");
        blockedUsersLevelHeader.innerHTML = "Stupeň blokování uživatelů";
        blockedUsersLevel = document.createElement("div");
        blockedUsersLevel.innerHTML = "0 - Neblokovat<br>1 - Skrýt text<br>2 - Skrýt příspěvek<br>3 - úplně odstranit";
        blockedUsersLevel.appendChild(customInput("setBlockedUserLevel", "blocked_user_level"));
        blockedUsersLevel.appendChild(customInput("setBlockedReactionLevel", "blocked_reaction_level"));
    }
    ...
    settingMenu.appendChild(blockedUsersLevelHeader);
    settingMenu.appendChild(blockedUsersLevel);
...
function barRoom() {
    if (debug)
        console.log("Vytvářím lištu v místnosti");
    var divBar = bar();
    ...
    nsfw.setAttribute("class", "bar_link");
    nsfw.setAttribute("onClick", "toggleNSFW(this);return false;");
    if (localStorage.getItem('nsfw') === "Y") {
        nsfw.innerHTML = "SFW";
    } else {
        nsfw.innerHTML = "NSFW";
    ...
    divBar.children[2].insertBefore(nsfw, divBar.children[2].firstElementChild);
...
function deleteJunk() {
    if (debug)
        console.log("Buduji si struktur webu a skrývám zbytečnosti");
...
function deleteJunkMainPage() {
    ...
    if (splitter.children[1] == null) { //Ma jen jedno dite
        ...
        if (debug)
            console.error("Splitter není tam kde má být!");
    }
...
function setStyles() {
    if (debug)
        console.log("Nastavuji styly");
    if (debug)
        console.groupCollapsed("Nastavování stylů");
...
function innerScripts()
{
    ...
    if (debug)
        console.log("Vytvá\u0159ím CSS dokument");
    if (debug)
        console.time("css zpracování. Split:" + splitCSS);
    ...
    if (debug)
        console.timeEnd("css zpracování. Split:" + splitCSS);
    if (debug)
        console.log('Zpracovávám Skripty');
    ...
    innerScripts += "\n " + setBlockedUserLevel.toString();
    innerScripts += "\n " + setBlockedReactionLevel.toString();
...
function showMessageField()
{
    ...
    if (debug)
        console.log("Načítám divy");
    var messageArea = document.getElementById("messageArea");
    var pitrisDiv = document.getElementById("pitrisDiv");
    var barLine;
    if (debug)
        console.log("Hledám pitrisDiv");
...
function toggleNSFW(node) {
    ...
    console.log(localStorage.getItem('nsfw'), nsfw);
...
{
    if (debug)
        console.log('Měním velikost písma');
...
{
    if (debug)
        console.log('Měním pozici lišty');
    if (position == "bottom") {
...
{
    if (debug)
        console.log('Měním barvu písma');
    editCSSRule(".post_text").style.color = textColor;
    localStorage.setItem("score_phorum_text_color", textColor);
}
function setBlockedUserLevel(level)
{
    if (debug)
        console.log('Měním level');
    localStorage.setItem("blocked_user_level", level);
}

function setBlockedReactionLevel(level)
{
    if (debug)
        console.log('Měním level');
    localStorage.setItem("blocked_reaction_level", level);
}
function setGrid(grid)
{
    if (debug)
        console.log('Měním nastavení mřížky');
    if (grid === "1")
...
{
    if (debug)
        console.log('Měním nastavení zakulacení:' + rounded);
    if (rounded != "yes") {
...
function setPageWidth(pageWidth) {
    if (debug)
        console.log('Měním šířku stránky');
...
function showSettings() {
    if (debug)
        console.log('Zobrazuji nastavení');
...
function showRoomList() {
    if (debug)
        console.log('Zobrazuji seznam místností');
...
function startup() {
    ...
    if (wParam == false || wParam == 1) {
        if (debug)
            console.log("Script je v seznamu místností");
        ...
        if (debug)
            console.log("Script je v v místnosti");
        innerScripts();
        deleteJunk();
        hideMessageArea();
        barRoom();
