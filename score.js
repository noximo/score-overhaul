...
function quickScroll(maxDuration = 800) {
  document.querySelectorAll('.jump-to-new').forEach(function(button) {
    ...
    clone.addEventListener('click', function() {
      ...
      var sibling = message.nextElementSibling;
      var next = null;
      while (sibling) {
        if (sibling.matches('.new-message, .message:not(.reply)')) {
          next = sibling;
          break;
        }
      ...
      if (next) {
        ...
        const target = next.getBoundingClientRect().top + start;
        const distance = Math.abs(target - start);
        const duration = Math.min(maxDuration, distance / 2);
        const startTime = Date.now();

        function scroll() {
          const elapsed = Date.now() - startTime;
          const progress = Math.min(elapsed / duration, 1);
          window.scrollTo(0, start + distance * (progress > 0.5 ? 1 - Math.pow(1 - progress, 2) : Math.pow(progress, 2)));
